...
**Consequences:** `db` must not re-fork ease/phase/unfamiliar rules; extract imports schedule. grade accepts `card_id` + `grade` + `sentence_id` (context shown). grade does not return the next Card — caller calls next again. Empty queue is a normal empty result, not an error.
